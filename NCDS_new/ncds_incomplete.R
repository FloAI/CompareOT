...
# ===============================================
# NCDS Incomplete Covariates Scenario
# ===============================================
...
cat("\nMerging databases (leaving covariates incomplete)... Please wait.\n")
# Merge datasets without forcing complete.cases or pre-imputing covariates
...
# Extract the prepared dataset (removing redundant identifier in col 4)
...
# --- B. kNN ---
cat("Running kNN...\n")
# kNN handles NAs in both covariates and targets naturally
...
meth[c("Y", "Z")] <- "rf" # Use random forest for categorical targets
...
# missForest will recursively impute covariates and targets
...
evaluate_classification <- function(df, target_var) {
  ...
  # CRITICAL FIX: na.roughfix fills leftover NAs with column medians/modes
  # This allows the Random Forest model to compile successfully when
  # testing methods like OTrecod that do not impute covariates.
  df <- randomForest::na.roughfix(df)
...
# ------------------------
write.csv(wass_results, "../wasserstein_distances_ncds_incomplete_scenario.csv", row.names = FALSE)
write.csv(final_class_results, "../classification_performance_ncds_incomplete_scenario.csv", row.names = FALSE)

cat("\nFiles successfully saved: 'wasserstein_distances_ncds_incomplete_scenario.csv' and 'classification_performance_ncds_incomplete_scenario.csv'.\n")