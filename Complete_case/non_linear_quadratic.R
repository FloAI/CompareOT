...
run_sim_combination <- function(num_samples, perc_missing, target_R2) {
  ...
  signal <- 20 + 5*X1_num + 1.5*(X1_num^2) + 10*X2_num - 2.5*(X2_num^2) + 2*X3_num - 8*X4 + 4*(X3_num * X4) + 0.3*X5
  ...
  df_mf <- mf_res$ximp
  res_mf <- calc_metrics(df_true, df_mf, m_mask_yb1, m_mask_yb2)

  # --- 2. kNN ---
  capture.output(df_knn <- kNN(df, k = 5, imp_var = FALSE))
  res_knn <- calc_metrics(df_true, df_knn, m_mask_yb1, m_mask_yb2)
  ...
  res_mice <- calc_metrics(df_true, df_mice, m_mask_yb1, m_mask_yb2)

  # --- 4. OTrecod ---
  capture.output(R_OUTC1 <- OT_joint(df, prox.X = 0.10, convert.num = 8, convert.class = 1,
                                     nominal = c(1,4:7), ordinal = 2:3, dist.choice = "H",
                                     maxrelax = 0.1, which.DB = "BOTH"))
  df_ot <- df
  df_ot$Yb2[is.na(df_ot$Yb2)] <- R_OUTC1$DATA1_OT[, "OTpred"]
  df_ot$Yb1[is.na(df_ot$Yb1)] <- R_OUTC1$DATA2_OT[, "OTpred"]
  res_ot <- calc_metrics(df_true, df_ot, m_mask_yb1, m_mask_yb2)

  # Combine results
  methods <- c("missForest", "kNN", "MICE", "OTrecod")
  return(data.frame(
    Method = methods,
    Precision = c(res_mf["Precision"], res_knn["Precision"], res_mice["Precision"], res_ot["Precision"]),
    Kappa = c(res_mf["Kappa"], res_knn["Kappa"], res_mice["Kappa"], res_ot["Kappa"]),
    MAE = c(res_mf["MAE"], res_knn["MAE"], res_mice["MAE"], res_ot["MAE"])
  ))
}
...
summary_results <- final_data %>%
  ...
  select(-starts_with("se_")) # Clean up standard error columns for final output

print(summary_results)


write.csv(final_data, "../simulation_qua_raw_results.csv", row.names = FALSE)
write.csv(summary_results, "../simulation_qua_summary_results.csv", row.names = FALSE)