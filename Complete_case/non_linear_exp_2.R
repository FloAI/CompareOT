...
run_sim_combination <- function(num_samples, perc_missing, target_R2) {
  ...
  signal <- exp(20 + 5*X1_num + 10*X2_num + 2*X3_num - 8*X4_num + 0.3*X5_num)
...
write.csv(final_data, "../simulation_raw_results_n_exp2.csv", row.names = FALSE)
write.csv(summary_results, "../simulation_summary_results_n_exp2.csv", row.names = FALSE)