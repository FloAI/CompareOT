...
run_sim_combination <- function(num_samples, perc_missing, target_R2) {
  ...
  X5_num <- X5
    ...
    # --- FIXED NON-LINEAR SIGMOID GENERATION ---
  # Center the linear predictor around 0 to utilize the steep slope of the sigmoid
  equation <- 5*X1_num + 10*X2_num + 2*X3_num - 8*X4 + 0.3*X5 - 29.5

  # Apply sigmoid and scale by 100 to map nicely to your categorization cutoffs
  signal <- 100 * (1 / (1 + exp(-equation)))

  # Safely calculate sigma (preventing division by zero if R2=1)
  if (target_R2 == 1) {
    sigma <- 0
  } else {
...
write.csv(final_data, "../simulation_raw_results_n_sig2.csv", row.names = FALSE)
write.csv(summary_results, "../simulation_summary_results_n_sig2.csv", row.names = FALSE)