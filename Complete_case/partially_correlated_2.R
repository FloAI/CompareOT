...
rho_list <- c(0.1, 0.5, 0.7)  # --- NEW: Added list of rho values ---
...
# ------------------------
run_sim_combination <- function(num_samples, perc_missing, target_R2, rho) { # --- NEW: Added rho argument ---
  ...
  # --- NEW: PARTIALLY CORRELATED COVARIATES ---
  # We induce a correlation using the passed rho specifically between X4 and X5
  cov_mat <- matrix(c(1, rho, rho, 1), nrow = 2)
  biv_norm <- mvrnorm(num_samples, mu = c(0, 0), Sigma = cov_mat)

  # X4 becomes binary (thresholding the first normal at 0 gives a ~50% split)
  X4 <- as.numeric(biv_norm[, 1] > 0)

  # X5 becomes uniform [20, 80] (using the standard normal CDF to transform it)
  X5 <- pnorm(biv_norm[, 2]) * 60 + 20
  # --------------------------------------------

  # Define signal numeric mappings
  X1_num <- X1
  X2_num <- ifelse(X2=="A",1,ifelse(X2=="B",2,0))
  X3_num <- as.numeric(factor(X3))

  # Non-Linear Sigmoid Generation
  equation <- 5*X1_num + 10*X2_num + 2*X3_num - 8*X4 + 0.3*X5 - 29.5
  signal <- 100 * (1 / (1 + exp(-equation)))
  ...
  Yb2 <- as.character(cut(Yb2_cont,
                          breaks = quantile(Yb2_cont, probs = c(0, 1/3, 2/3, 1)),
...
for (n_samp in num_samples_list) {
  for (p_miss in perc_missing_list) {
    for (R2 in target_R2_list) {
      for (current_rho in rho_list) { # --- NEW: Loop through rho values ---

        for (rep_i in 1:n_reps) {
          sim_res <- tryCatch({
            run_sim_combination(n_samp, p_miss, R2, current_rho)
          }, error = function(e) {
            message(sprintf("Error in rep %d (n=%d, miss=%.1f, R2=%.1f, rho=%.1f): %s", rep_i, n_samp, p_miss, R2, current_rho, e$message))
            return(NULL)
          })
          ...
          if (!is.null(sim_res)) {
            ...
            sim_res$rho <- current_rho # --- NEW: Store rho in results ---
            results_list[[length(results_list) + 1]] <- sim_res
          }
        }
        cat(sprintf("Completed: Samples=%d, Missing=%.1f, R2=%.1f, Rho=%.1f\n", n_samp, p_miss, R2, current_rho))
      }
    }
  }
}
...
summary_results <- final_data %>%
  group_by(num_samples, perc_missing, target_R2, rho, Method) %>% # --- NEW: Group by rho ---
  summarize(
...
write.csv(final_data, "../simulation_raw_results_n_sig2_rho.csv", row.names = FALSE)
write.csv(summary_results, "../simulation_summary_results_n_sig2_rho.csv", row.names = FALSE)