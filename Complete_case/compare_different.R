...
run_sim_combination <- function(num_samples, perc_missing, target_R2) {
  ...
  signal1 <- 20 + 5*X1_num + 10*X2_num + 2*X3_num - 8*X4 + 0.3*X5
  signal2 <- 15 + 4*X1_num + 8*X2_num + 1.5*X3_num- 6*X4 + 0.25*X5


  # Compute sigma for target R²
  sigma1 <- sqrt(var(signal1) * (1/target_R2 - 1))
  sigma2 <- sqrt(var(signal2) * (1/target_R2 - 1))
  ...
  capture.output(R_OUTC1 <- OT_joint(df, prox.X = 0.10, convert.num = 8, convert.class = 1,
                                     nominal = c(1,4:7), ordinal = 2:3, dist.choice = "E",
  ...
  res_mda <- calc_metrics(df_true, df_mda, m_mask_yb1, m_mask_yb2)
...
write.csv(final_data, "../simulation_raw_results_D.csv", row.names = FALSE)
write.csv(summary_results, "../simulation_summary_results_D.csv", row.names = FALSE)